...
async function run(hre, admin, contract, func, args, options={}) {
    var request = {
        signer: admin.signer,
        contract: contract,
        func: func,
    ...
    const argValues = args.map(([k, v]) => normalize(v));
...
async function clone(hre, signer, base, initArgs) {
    ...
    const { topics: [_hash, _from, cloned] } = logs.find(
        l => ethers.BN.from(l.topics[1]).eq(
            ethers.BN.from(base.address)
        )
    );
