    ...
    return async (...args) => {
        ...
        if (!results[argsKey] || results[argsKey].expiredAt < ts) {
            results[argsKey] = {
                ...
                expiredAt: ts + 30000 // retain for 30s
            }
        }
...
async function query(config, endpoint, data, method) {
    ...
    axiosRetry(
        instace,
        {
            ...
            retryCondition: (error) => {
                logger.error('axios query error: ' + JSON.stringify(error));
    ...
    logger.debug(
        'Query response: ' + JSON.stringify(response.data, null, 2)
    );
    return response.data.data;
}

const account = memoize(
    'account',
    async (config, coin) => {
...
const userHashrateChart = memoize(
    'userHashrateChart',
...
const paymentHistoryV2 = memoize(
    'paymentHistoryV2',
    async (config, coin) => {
...
async function statsYesterday(config, coin) {
    ...
    return {
        ...
        totalEarned: new BigNumber(acc.earn24Hours),
        date: acc.settleTime
    };
}
