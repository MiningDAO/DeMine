...
async function released(erc1155, raw) {
    const custodian = await erc1155.custodian();
    const ids = encode(raw);
    const balances = await erc1155.supplyOfBatch(ids);
    const locked = await erc1155.balanceOfBatch(
        Array(ids.length).fill(custodian),
        ids
    );
    var tokenSupply = ethers.BigNumber.from(0);
    var tokenReleased = ethers.BigNumber.from(0);
    for (let i = 0; i < ids.length; i++) {
        tokenSupply = tokenSupply.add(balances[i]);
        tokenReleased = tokenReleased.add(balances[i]).sub(locked[i]);
    }
    return {tokenReleased, tokenSupply};
}

async function supplyOf(erc1155, endOfDay) {
    ...
    return await released(erc1155, ids);
}
