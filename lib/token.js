...
async function released(ethers, erc1155, raw) {
    const custodian = await erc1155.custodian();
    const ids = encode(ethers, raw);
    const balances = await erc1155.supplyOfBatch(ids);
    const locked = await erc1155.balanceOfBatch(
        Array(ids.length).fill(custodian),
        ids
    );
    var supply = ethers.BigNumber.from(0);
    var released = ethers.BigNumber.from(0);
    for (let i = 0; i < ids.length; i++) {
        supply = supply.add(balances[i]);
        released = released.add(balances[i]).sub(locked[i]);
    }
    return {released, supply};
}
...
async function supplyOf(ethers, erc1155, endOfDay) {
    ...
    return await released(ethers, erc1155, ids);
}
