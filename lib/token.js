...
async function liquidized(ethers, erc1155, raw) {
    ...
    var res = ethers.BigNumber.from(0);
    for (let i = 0; i < ids.length; i++) {
        res = res.add(balances[i].sub(locked[i]));
    }
    return res;
}
...
async function supplyOf(ethers, erc1155, endOfDay) {
    ...
    return await liquidized(ethers, erc1155, ids);
}
